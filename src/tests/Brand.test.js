...
describe("Brand Component Test Cases", () => {
  ...
  it("1 :Should render all the elements correctry", async () => {
    render(
  ...
  it("5 :Should delete segment(id 1) and alse from list", async () => {
    render(
      <Provider store={store}>
        <Brand />
      </Provider>
  ...
  it("6 :Should delete segment(id 2) and alse from list", async () => {
    render(
      <Provider store={store}>
        <Brand />
      </Provider>
  ...
  it("7 :Should update segment(id 1) and alse in the list", async () => {
    render(
      <Provider store={store}>
        <Brand />
      </Provider>
    ...
    expect(screen.queryByText("Toyota")).toBeNull();
    expect(screen.queryByText("Tesla")).toBeNull();
    expect(screen.getByTestId("btn-post").textContent).toBe("Create");
    expect(await screen.findByText("Toyota")).toBeInTheDocument();
    expect(screen.getByTestId("list-2").textContent).toBe("Tesla");
    userEvent.click(screen.getByTestId("edit-brand-1"));
    expect(screen.getByTestId("btn-post").textContent).toBe("Update");
    userEvent.click(screen.getByTestId("btn-post"));
  ...
  it("8 :Should update segment(id 2) and alse in the list", async () => {
    render(
      <Provider store={store}>
        <Brand />
      </Provider>
